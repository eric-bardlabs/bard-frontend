  ...
  return (
    <header className="p-4 border-b border-divider">
      ...
      <div className="md:hidden space-y-4">
        {/* Top row - Main actions and view switcher */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          {/* Left side - Actions */}
          <div className="flex items-center gap-2 flex-wrap">
            <Button color="primary" onPress={openNewSessionModal} size="sm">
              <Icon icon="lucide:plus" className="mr-1" />
              Create
            </Button>
            
            <Button variant="light" onPress={handleToday} size="sm">
              Today
            </Button>
            
            <div className="flex items-center">
              <Button isIconOnly variant="light" onPress={handlePrevious} size="sm">
                <Icon icon="lucide:chevron-left" />
              </Button>
              <Button isIconOnly variant="light" onPress={handleNext} size="sm">
                <Icon icon="lucide:chevron-right" />
              </Button>
            </div>
        ...
        {/* Bottom row - Date picker and status */}
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div className="flex flex-col">
            <DatePicker
              value={currentDate}
          ...
          {isLoading && (
            <div className="flex items-center gap-2 text-sm text-default-500">
              <Spinner size="sm" />
              <span className="hidden sm:inline">Loading events...</span>
            </div>
          )}
        </div>
      </div>
