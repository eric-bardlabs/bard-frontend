...
type Props = {
  defaultSelected?: CollaboratorSelection[];
...
export function CollaboratorMultiSelect({
  ...
  const [selected, setSelectedInternal] = useState<CollaboratorSelection[]>(defaultSelected);
  ...
  const {
    ...
    queryKey: ["collaborators-select", debouncedSearch],
  ...
  // Convert collaborators to options and filter out already selected ones
  const availableOptions: CollaboratorSelection[] = useMemo(() => {
    const selectedIds = new Set(selected.map(s => s.id));
    return allCollaborators
      .filter(collab => !selectedIds.has(collab.id))
      .map(collab => ({
        id: collab.id,
        label: collab.artist_name || collab.legal_name || "Unknown",
        subtitle: collab.email,
        avatar: undefined, // Add avatar URL if available in your data
      }));
  }, [allCollaborators, selected]);

  const handleUnselect = useCallback((option: CollaboratorSelection) => {
              ...
              <div className="py-8 text-center text-default-500">
                ...
                <p>
                  {maxItems !== undefined && selected.length >= maxItems
                    ? `Maximum of ${maxItems} collaborators selected.`
                    : debouncedSearch 
